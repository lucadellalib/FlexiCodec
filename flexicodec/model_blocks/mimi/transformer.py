...
class StreamingTransformerLayer(StreamingModule[_LayerState]):
    ...
    def __init__(
        ...
        weights_per_step: int = 0,
        activation=F.gelu,
        skip_self_attn: bool = False, # NOTE: use transformer or not
        device=None,
        dtype=None,
    ):
        super().__init__()
...
class QueryTokenAggregator(nn.Module):
    ...
    def __init__(self, dim: int, in_out_dim: int, num_heads: int, num_layers: int, dim_feedforward: int, causal: bool = False, context_frames: int = 125, use_mean_pooling_init: bool = True):
        super().__init__()
        ...
        self.transformer = ProjectedTransformer(**transformer_kwargs)
